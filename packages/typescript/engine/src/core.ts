...
export interface CompileOptions {
  strict?: boolean;
  allowUnresolvedDocuments?: boolean;
}
...
export function initialSnapshot(project: Project): WorkspaceSnapshot {
  return {
    values: Object.fromEntries(
      project.workspace.variables.map((variable) => [
        variable.key,
...
export function applyPatch(
  ...
  options: {
    allowedWrites: Iterable<string>;
    constKeys: Iterable<string>;
    bitId?: string;
    strictUndeclaredAccess?: boolean;
  },
): [WorkspaceSnapshot, WorkspacePatch] {
  ...
  for (const operation of patch.ops) {
    if (
      !allowed.has(operation.key) &&
      options.strictUndeclaredAccess !== false
    ) {
      audited.push({
        op: "reject",
        key: operation.key,
...
export function parseRule(data: unknown): RuleAst {
  if (!data || typeof data !== "object" || Array.isArray(data))
    throw new RuleTypeError("Rule must be an object");
  ...
  if (
    ...
    throw new RuleTypeError("Invalid rule AST");
  return rule;
}
function variableName(rule: unknown): string | undefined {
  return typeof rule === "string"
    ...
    : rule &&
        typeof rule === "object" &&
        (rule as RuleAst).op === "var" &&
        typeof (rule as RuleAst).args[0] === "string"
      ? ((rule as RuleAst).args[0] as string)
      : undefined;
}
export function typecheckRule(
  rule: RuleAst,
  variables: Map<string, { defaultValue: WorkspaceValue }>,
): void {
  const check = (node: RuleAst, comparable = false): void => {
    if (node.op === "var") {
      const key = variableName(node);
      if (!key || !variables.has(key))
        throw new RuleTypeError(`Unknown variable: ${key ?? ""}`);
      if (comparable && variables.get(key)?.defaultValue.kind === "secret_ref")
        throw new RuleTypeError(`Secret variable cannot be compared: ${key}`);
    }
    const comparison = ["eq", "ne", "lt", "le", "gt", "ge"].includes(node.op);
    for (const argument of node.args)
      if (argument && typeof argument === "object" && "op" in argument)
        check(argument as RuleAst, comparable || comparison);
  };
  check(rule);
}
function unwrap(value: WorkspaceValue): unknown {
  if (value.kind !== "public_literal")
    throw new RuleEvalError(`Cannot evaluate ${value.kind}`);
  return value.value;
}
  ...
  const evaluate = (node: RuleAst): unknown => {
    ...
    if (node.op === "var") {
      ...
      if (!value) {
        ...
        throw new RuleEvalError(`Missing variable: ${key}`);
      }
      return unwrap(value);
    }
    if (node.op === "has")
      return Boolean(workspace.values[variableName(args[0]) ?? ""]);
    if (node.op === "not") return !Boolean(evaluate(args[0] as RuleAst));
    if (node.op === "and")
      return args.every((arg) => Boolean(evaluate(arg as RuleAst)));
    if (node.op === "or")
      return args.some((arg) => Boolean(evaluate(arg as RuleAst)));
    ...
    if (
      ...
      throw new RuleEvalError("Ordered comparison requires compatible values");
    if (node.op === "lt")
