...
export const workspaceValueSchema = z.discriminatedUnion("kind", [
  z.object({
    kind: z.literal("public_literal"),
    dataType: z.enum(["string", "number", "boolean", "date"]),
    value: z.unknown(),
  }),
  z.object({
    kind: z.literal("secret_ref"),
    ref: z.string().regex(/^sec:.+/),
    displayHint: z.string().optional(),
  }),
  z.object({
    kind: z.literal("eq_token"),
    token: z.string(),
    domain: z.string().optional(),
  }),
  z.object({
    kind: z.literal("shape_token"),
    shape: z.enum(["email", "phone", "iban", "uuid", "code"]),
  ...
  z.object({
    ...
    reason: z.enum(["secret", "policy", "consent", "unknown"]),
  }),
]);
...
export const bitSchema = z
  .object({
    ...
    when: z.union([z.literal(true), ruleSchema]),
    config: z.record(z.unknown()),
    reads: z.array(z.string()),
    writes: z.array(z.string()),
    emits: z.array(z.string()),
    effectClass: z.string(),
    onError: z.enum(["abort", "skip", "continue"]),
...
export const projectSchema = z
  .object({
    ...
    pluginRequirements: z.array(
      z
        .object({ name: z.string().min(1), versionRange: z.string().min(1) })
        .strict(),
    ),
    workspace: z
      .object({
